...
`Flowcraft.html` from the earlier conversation is a 1.0 offline build and is now stale relative to the hosted 1.1 interface. Rebuild a fresh standalone from canonical source with `python build.py`. Do not hand-edit generated distributions. The downloadable source bundle excludes all private user exports and private test fixtures.
