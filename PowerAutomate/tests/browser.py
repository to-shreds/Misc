...
with sync_playwright() as pw:
    ...
    page.get_by_text('Preview email layout',exact=True).click()
    ...
    page.get_by_role('button',name=re.compile(r'Loop \+ collect results')).click()
    loop=node_name(page,'loop')
    page.locator('.field').filter(has=page.locator('label',has_text='Repeat for each item')).get_by_role('button',name='Insert value').click()
